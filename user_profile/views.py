...
def user_fav_view(request):
    ...
    context = dict(
        ...
        # favs=request.user.userpostfav_set.filter(is_deleted=False).order_by('-updated_at'),
    )
...
def register_view (request):
    ...
    if request.method == "POST":
        ...
        if email != email_confirm:
            messages.warning(request, "lütfen email bilgisini dogru giriniz.")
            return redirect('user_profile:register_view')
        
        if password != password_confirm:
            messages.warning(request, "lütfen sifre bilgisini dogru giriniz.")
            return redirect('user_profile:register_view')
        
        user, created = User.objects.get_or_create(username=email)
        #eger kullanici created değilse kullanici daha önce sisteme kayitlidir
        if not created:
            user_login=authenticate(request, username=email, password=password)
            if user is not None:
                messages.success(request, "Daha önce kayit olmussunuz.. Anasayfaya yönlendirildiniz..")
                #kullanici login oldu ;)
                login(request, user_login)
                return redirect('home_view')
            messages.warning(request, f'{email} adresi sistemde kayitli ama login olmadiniz.. Login sayfasina yonlendiriliyorsunuz')
            return redirect('user_profile:login_view')
        user.email = email
